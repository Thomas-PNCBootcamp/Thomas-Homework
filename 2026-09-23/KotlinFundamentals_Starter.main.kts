...
// balance (Double).
data class Account(val id:String, val name:String,val balance:Double)
...
sealed class TransferResult{
 ...
 fun describe(result:TransferResult):String =
        when(result){
            is TransferResult.Success -> "The request was a Success: ${result.confirmationId}"
            is TransferResult.Failure -> "The request was a fail: ${result.reason}"
        }
...
suspend fun fetchAccounts():List<Account>{
    delay(500)
    val accounts = listOf(
        Account("101","John Doe", 323.43),
        Account("213","Jane Doe", 1273.83),
        Account("135","Blake Dean", -896.79),
...
fun main() = runBlocking {
    ...
    val negativeAccounts = fetchedAccounts.filter{it.balance < 0.0}
                        .map{"${it.name} (${it.balance.asCurrency()})"}
    val totalBalance = fetchedAccounts.sumOf{it.balance}
    val formattedBalance = totalBalance.asCurrency()
    println("Accounts with negative balances: $negativeAccounts")
    println("total balance across all accounts: $formattedBalance")
